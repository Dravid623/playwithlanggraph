import {retriever, vectorStore} from "./vectorStore.js";
...
const GraphState = Annotation.Root({
  ...
  inputToken: Annotation<number>({
    reducer:(x,y)=>x+y,
    default: ()=> 0
  }),
  outputToken: Annotation<number>({
    reducer:(x,y)=>x+y,
    default: ()=> 0
  })
})
...
// console.log("score output at grade documet================================================",score.usage_metadata?.input_tokens)
  return {
    messages: [score],
    inputToken: score.usage_metadata?.input_tokens,
    outputToken: score.usage_metadata?.output_tokens
  };
}
...
//   console.log("response output at rewrite================================================",response.usage_metadata?.input_tokens)
  return {
    messages: [response],
    inputToken: response.usage_metadata?.input_tokens,
    outputToken: response.usage_metadata?.output_tokens
  };
}
